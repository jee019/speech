function doGet(e) {
	...
	var SpreadSheet = SpreadsheetApp.openById("YOUR_ENG_SHEET_ID_HERE");  // ENG용 Sheet ID로 변경 필요
	var Sheet = SpreadSheet.getSheets()[0];
	var LastRow = Sheet.getLastRow();

	// Collect all question answers (ENG only: eng_nmos_q1, eng_smos_q1, etc.)
	var allQuestions = [];
	...
	for (var key in params) {
		...
		// Collect only ENG question parameters (eng_nmos_q1, eng_smos_q1, etc.)
		if (key.match(/^eng_(nmos_|smos_)q\d+$/)) {
			allQuestions.push({
				key: key,
				value: params[key]
			});
	...
	// Sort questions: ENG NMOS -> ENG SMOS
	allQuestions.sort(function(a, b) {
		// Extract prefix (eng_nmos_, eng_smos_)
		var prefixA = a.key.match(/^eng_(nmos_|smos_)/);
		var prefixB = b.key.match(/^eng_(nmos_|smos_)/);
		...
		var numA = parseInt(a.key.replace(/^eng_(nmos_|smos_)?q/, ''));
		var numB = parseInt(b.key.replace(/^eng_(nmos_|smos_)?q/, ''));
		return numA - numB;
	});
	...
	// Structure: Name | ENG NMOS (60개) | ENG SMOS (60개)
	...
	for (var i = 0; i < allQuestions.length; i++) {
		...
		// Extract prefix
		var prefixMatch = question.key.match(/^eng_(nmos_|smos_)/);
		var currentPrefix = prefixMatch ? prefixMatch[0] : '';
		
		// Add blank column when transitioning from NMOS to SMOS
		if (previousPrefix === 'eng_nmos_' && currentPrefix === 'eng_smos_') {
